...
public class CongTy implements NhapXuat{
	...
	//1. attributes
		...
		// Câu 3: Phân bổ nhân viên về phòng ban. Xuất thông tin DS Nhân Viên
		public void phanBoNhanVien() {
			// Duỵệt danh sách nhân viên // lấy danh sách nhân viên trước
					...
					do {
						...
						case 1:
							...
							if (pb != null) {
								...
								pb.getDsNhanVien().add((NhanVienThuong) ns); // ép kiểu
							}
							flag = false;
							break;
						case 2:
		...
		private PhongBan layPhongBan(String maPhong) { // lấy phòng ban dự vào mã phòng
		...
		//Câu 4: Chỉ định Trưởng phòng quản lý phòng ban
		public void chiDinhTruongPhong() {
			// Duỵet danh sách Truong Phòng // lấy danh sách Truong Phòng trước 
			// Hỏi muốn phân bổ hay không,
			// 1: Phân bổ -> Liệt kê danh sách phòng ban
			// Chọn nhập vào mã phòng ban
			// 2: Đi tiếp không phân bổ
			...
			do {
				...
				for (NhanSu nSu : this.objDSNhanSu.getListNhanSu()) {
					if (nSu.getMaNV().equalsIgnoreCase(ma)) {
						...
						ns.xuatTenVaMa();
						break; // tìm 1 nhân sự nên mới break;
					}
				}
			} while (ns == null);
			return ns;
